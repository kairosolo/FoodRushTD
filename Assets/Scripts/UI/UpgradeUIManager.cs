...
public class UpgradeUIManager : MonoBehaviour
{
    ...
    private void RefreshPanel()
    {
        ...
        stationInfoText.text = $"Level: {currentStation.SpecializationLevel + 1}\nSpeed Bonus: +{currentSpeedBonus:F0}%";

        bool canSpecialize = currentStation.CanUpgradeSpecialization();
        specializeButton.interactable = canSpecialize;
        if (canSpecialize)
        {
            specializeButtonText.text = $"FASTER!!\n${currentStation.GetSpecializeCost()}";
        }
        else
        {
            specializeButtonText.text = "Max Level";
        }
        ...
        if (canDiversify)
        {
            diversifyButtonText.text = $"Learn New Recipe\n${currentStation.StationData.DiversifyCost}";
        }
        else
        {
            diversifyButtonText.text = "Fully Diversified";
        }
